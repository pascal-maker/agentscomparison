...
class ReviewersAgent:
    """
    Represents a group of reviewer agents that assess the final research report.
    These agents critique the originality, quality, clarity, and significance
    of the work, simulating the peer-review process.
    """
...
class BaseAgent:
    """
    Base class for all research agents in the laboratory.
    Maintains the state, history, and context across different research phases,
    and handles the core inference loops and prompt construction.
    """
...
class ProfessorAgent(BaseAgent):
    """
    Represents a computer science professor overseeing the project.
    Primarily involved in the 'report writing' phase, directing the PhD student
    to ensure the final paper meets high academic standards.
    """
...
class PostdocAgent(BaseAgent):
    """
    Represents a postdoctoral researcher providing guidance.
    Involved in 'plan formulation' and 'results interpretation',
    helping to devise innovative experimental plans and interpret outcomes.
    """
    def __init__(self, model="gpt4omini", notes=None, max_steps=100, openai_api_key=None):
        super().__init__(model, notes, max_steps, openai_api_key)
...
class MLEngineerAgent(BaseAgent):
    """
    Represents a Machine Learning Engineer executing the technical plan.
    Responsible for 'data preparation' and 'running experiments',
    writing code, interacting with datasets, and generating models.
    """
    def __init__(self, model="gpt4omini", notes=None, max_steps=100, openai_api_key=None):
        super().__init__(model, notes, max_steps, openai_api_key)
        self.phases = [
...
class SWEngineerAgent(BaseAgent):
    """
    Represents a Software Engineer assisting with data and infrastructure.
    Works closely with the ML Engineer during the 'data preparation' phase
    to ensure scripts and datasets are properly established.
    """
    def __init__(self, model="gpt4omini", notes=None, max_steps=100, openai_api_key=None):
        super().__init__(model, notes, max_steps, openai_api_key)
        self.phases = [
...
class PhDStudentAgent(BaseAgent):
    """
    Represents a PhD student who orchestrates the primary research workflow.
    Involved in almost all phases including literature review, planning,
    coordinating experiments, interpreting results, and writing the report.
    """
    def __init__(self, model="gpt4omini", notes=None, max_steps=100, openai_api_key=None):
        super().__init__(model, notes, max_steps, openai_api_key)
        self.phases = [
