...
def query_deepseekv3(prompt, system, api_key, attempt=0, temperature=0.0):
    """
    Queries the DeepSeek v3 API using the provided system and user prompts.
...
def query_qwen(prompt, system, api_key, attempt=0, temperature=0.0):
    """
    Queries the Qwen/QwQ-32B model via HuggingFace Inference API.
    Retries up to 10 times in case of failure.
    """
    try:
...
def query_gpt4omini(prompt, system, api_key, attempt=0, temperature=0.0):
    """
    Queries the OpenAI gpt-4o-mini API using the provided system and user prompts.
    Retries up to 10 times in case of failure.
    """
    try:
...
def query_gpt4o(prompt, system, api_key, attempt=0, temperature=0.0):
    """
    Queries the OpenAI gpt-4o API using the provided system and user prompts.
    Retries up to 10 times in case of failure.
    """
    try:
        openai_api_key = api_key
...
def query_gemini(prompt, system, api_key, attempt=0, temperature=0.0):
    """
    Queries the Google Gemini 1.5 Pro API using the provided system and user prompts.
    Retries up to 10 times in case of failure.
    """
    try:
...
def compile_latex(latex_code, output_path, compile=True, timeout=30):
    """
    Compiles LaTeX code into a PDF by writing it to a temporary file 
    and invoking pdflatex via subprocess.
    Includes basic LaTeX packages by default.
    """
